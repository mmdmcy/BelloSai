...
function GameComponent({ isDark, customization, onToggleTheme }: GameProps) {
  ...
  const generateQuestion = async () => {
    ...
    try {
      const prompt = `Generate a Family Feud question with exactly 5 answers and points. Format:
Question: [question]
Answers:
1. [answer] - [points]
2. [answer] - [points]
...
Make it fun and modern. No markdown formatting.`;
      ...
      const response = await sendChatMessage([{ type: 'user', content: prompt }], 'DeepSeek-V3');
